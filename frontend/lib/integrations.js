...
//    (see next.config.mjs rewrites) - the browser never talks to the
//    backend cross-origin and never sees credentials.
...
export async function fetchIntegrationStatuses() {
  const response = await fetch(`${BACKEND_PROXY_PREFIX}/api/integrations`, {
    cache: 'no-store'
  });
  if (!response.ok) {
...
export async function connectIntegration(integrationId) {
  const response = await fetch(
    `${BACKEND_PROXY_PREFIX}/api/integrations/${encodeURIComponent(integrationId)}/connect`,
    { method: 'POST' }
  );
